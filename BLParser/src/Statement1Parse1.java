...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isCondition(c),
                "Violation of: condition is valid.");
        Condition c2 = parseCondition(c);
        String then = tokens.dequeue();
        Reporter.assertElseFatalError(then.equals("THEN"), "Need THEN.");
        ...
        if (tokens.front().equals("ELSE")) {
            ...
            s.assembleIfElse(c2, ifBlock, elseBlock);
        } else {
            s.assembleIf(c2, ifBlock);
        }
        tokens.dequeue();
        String token = tokens.dequeue();
        Reporter.assertElseFatalError(token.equals("IF"),
                "Error: Expect is END IF");
    ...
    private static void parseWhile(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isCondition(tokens.front()),
                "Violation of: condition is invalid");
        Condition c = parseCondition(tokens.dequeue());
        Reporter.assertElseFatalError(tokens.front().equals("DO"), "Need DO");
        tokens.dequeue();
        Statement whileBlock = s.newInstance();
        whileBlock.parseBlock(tokens);
        s.assembleWhile(c, whileBlock);
        Reporter.assertElseFatalError(tokens.front().equals("END"), "NEED END");
        tokens.dequeue();
    ...
    private static void parseCall(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isIdentifier(tokens.front()),
                "You should expect a identifier");
    ...
    public void parse(Queue<String> tokens) {
        ...
        Reporter.assertElseFatalError(
                tokens.front().equals("WHILE") || tokens.front().equals("IF")
                        || Tokenizer.isIdentifier(tokens.front()),
                "You should expect WHILE,IF OR IDENTIFIER");
        //parse While
    ...
    public void parseBlock(Queue<String> tokens) {
        ...
        for (int i = 0; tokens.front().equals("WHILE")
                || tokens.front().equals("IF")
                || Tokenizer.isIdentifier(tokens.front()); i++) {
            body.parse(tokens);
            this.addToBlock(i, body);
        }
